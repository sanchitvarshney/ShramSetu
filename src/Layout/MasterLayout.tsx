...
const SIDEBAR_ICONS: Record<string, React.ReactNode> = {
 ...
 DEPARTMENT_LIST: <IoList className="h-[20px] w-[20px]" />,
 DEPARTMENT_CREATE: <IoPersonAddOutline className="h-[20px] w-[20px]" />,
 DESIGNATION_LIST: <IoList className="h-[20px] w-[20px]" />,
 DESIGNATION_CREATE: <IoPersonAddOutline className="h-[20px] w-[20px]" />,
    ...
    <div className="h-[calc(100vh-70px)] w-[calc(100vw-20px)] grid grid-cols-[300px_1fr]">
      <div className="w-full border p-[10px] ">
        <nav className="w-full flex flex-col gap-[5px]">
          {Master_SIDEBAR_ORDER.map((key) => {
            const route = APP_ROUTES[key];
            if (!route?.inAdminSidebar)
              return null;
